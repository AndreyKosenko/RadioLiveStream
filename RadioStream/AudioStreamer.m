...
void MyAudioSessionInterruptionListener(void *inClientData, UInt32 inInterruptionState)
{
	AudioStreamer *radio = (__bridge AudioStreamer*)inClientData;
...
{
	AudioStreamer *myData = (__bridge AudioStreamer *)dataIn;
...
//	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
	...
	OSStatus err = AudioFileStreamOpen((__bridge void * _Nullable)(self), MyPropertyListenerProc, MyPacketsProc, 
...
-(void) changeVolume:(float) percent{
    AudioQueueSetParameter(audioQueue, kAudioQueueParam_Volume, percent);
}
